...
function PopularMovies() {
  const [moviePopular, setPopular] = useState([]);

  const fetchPopularMovies = async () => {
    try {
      const { data } = await axios.get(
        "https://api.themoviedb.org/3/movie/popular?api_key=1b074e6381e134256262bd2ad50cf047"
      );
    ...
    <div className="font-Raleway relative">
      ...
      <div className="relative flex overflow-x-scroll scroll-smooth space-x-10 pl-10 pr-5 py-5 mt-5 [&::-webkit-scrollbar]:hidden flex-nowrap">
        {moviePopular.map((movie) => (
          ...
          >
          <div  className="flex-shrink-0 w-40 min-w-[200px] cursor-pointer">
            <img
